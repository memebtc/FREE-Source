...
#if defined(NDEBUG)
# error "FREE cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xff6b184e15aa8c51f23c6bb868e6e94545b24e342c52d0be9bd4b059b57e3ad0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // FREE: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "FREE Signed Message:\n";
...
{
    // FREE: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // FREE: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // FREE: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1512926119, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Free is free now 2017 12 10";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 500 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04cc7c666b34aee662f1c0063d43c62355fa56cb1369f07cd8583451da65e1b11c7d19cf8e17ed0aba1617bac0737f830d0f1e550954e22481d70566ddccb2d337") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1512926119;
        ...
        assert(block.hashMerkleRoot == uint256("0x39db1973e7cbb307114b8c48eaed44e14563ea3159b2b6166572982a1b3b8da5"));
...
unsigned char pchMessageStart[4] = { 0xd5, 0x4f, 0x33, 0x74 }; // FREE: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FREEMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FREEMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FREEMiner(CWallet *pwallet)
{
    printf("FREEMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("free-miner");
    ...
    try { loop {
        ...
        printf("Running FREEMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FREEMiner, pwallet));
}
